...
- ASDFormer: A Transformer with Mixtures of Pooling-Classifier Experts for   Robust Autism Diagnosis and Biomarker Discovery
  ...
  - Code: Not available
  - Summary: The research paper introduces ASDFormer, a novel Transformer architecture that leverages a Mixture of Pooling-Classifier Experts (MoE) to enhance the diagnosis of Autism Spectrum Disorder (ASD) and facilitate biomarker discovery. By integrating multiple specialized expert branches with attention mechanisms, ASDFormer effectively captures critical neural signatures and connectivity patterns associated with ASD, leading to improved classification accuracy and greater interpretability of disorder-related biomarkers. When tested on the ABIDE dataset, ASDFormer demonstrates state-of-the-art performance in diagnostic accuracy and provides valuable insights into functional connectivity disruptions linked to ASD. #MixtureOfExperts #MoE #AutismDiagnosis #BiomarkerDiscovery
  - 摘要: 本研究提出了ASDFormer，一种基于Transformer的架构，结合了混合池化分类专家（MoE）以捕捉与自闭症谱系障碍（ASD）相关的神经特征。通过整合多个专门的专家分支和注意力机制，ASDFormer能够自适应地强调与自闭症相关的不同脑区和连接模式，从而提升分类性能并更清晰地识别与疾病相关的生物标志物。在ABIDE数据集上的应用表明，ASDFormer达到了最先进的诊断准确率，并揭示了与ASD相关的功能连接中断的深刻见解，展示了其作为生物标志物发现工具的潜力。  
#混合专家 #MoE #效率
