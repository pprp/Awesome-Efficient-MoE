...
- FakeSV-VLM: Taming VLM for Detecting Fake Short-Video News via   Progressive Mixture-Of-Experts Adapter
  - Authors: Junxi Wang, Yaxiong Wang, Lechao Cheng, et al.
  - Link: http://arxiv.org/pdf/2508.19639v1
  - Code: Not available
  - Summary: The research paper introduces FakeSV-VLM, a novel framework leveraging Vision Language Models (VLMs) to enhance the detection of fake news in short videos. The authors propose a Progressive Mixture-Of-Experts Adapter (PMOE) that employs four specialized experts to analyze distinct scenarios of video and text authenticity, improving detection accuracy through a two-step analysis process. Additionally, they introduce the Alignment-driven Event Checking (ADEC) module to address inconsistencies between modalities, demonstrating significant performance improvements on benchmark datasets. #MixtureOfExperts #MoE #FakeNewsDetection #VLM #MachineLearning
  - 摘要: 本文提出了一种基于视觉语言模型（VLM）的新框架FakeSV-VLM，用于检测短视频平台上的虚假新闻。通过分析短视频样本的四种不同场景，设计了四个专门的专家来处理每种情况，并通过混合专家（MoE）集成到VLM中。研究还开发了渐进式MoE适配器（PMOE）和基于对齐的事件检查（ADEC）模块，以提高检测的准确性和鲁棒性。实验结果表明，该方法在两个基准数据集上表现优异。#混合专家 #MoE #效率  - Authors: Yuan Xie, Shaohan Huang, Tianyu Chen, Furu Wei
