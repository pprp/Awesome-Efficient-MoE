...
- Federated Fine-Tuning of Sparsely-Activated Large Language Models on   Resource-Constrained Devices
  - Authors: Fahao Chen, Jie Wan, Peng Li, et al.
  - Link: http://arxiv.org/pdf/2508.19078v1
  - Code: Not available
  - Summary: The paper presents FLUX, a novel system for federated fine-tuning of Mixture-of-Experts (MoE)-based large language models (LLMs) on resource-constrained devices, addressing the challenges posed by high computational demands. FLUX introduces three key innovations: quantization-based local profiling for efficient expert activation estimation, adaptive layer-aware expert merging to optimize resource use while maintaining accuracy, and dynamic expert role assignment to balance tuning efforts. Experimental results on LLaMA-MoE and DeepSeek-MoE show that FLUX achieves up to 4.75X speedup in time-to-accuracy compared to existing methods, demonstrating its effectiveness in real-world applications. #MixtureOfExperts #MoE #Efficiency
  - 摘要: 本文提出了一种名为FLUX的系统，旨在解决在资源受限设备上进行混合专家模型（MoE）的大型语言模型（LLM）联邦微调的挑战。FLUX通过引入量化基础的本地分析、适应性层感知专家合并和动态专家角色分配等三项创新，显著提高了微调效率，最大限度地减少了时间到达准确性的时间。实验结果表明，FLUX在LLaMA-MoE和DeepSeek-MoE上相较于现有方法实现了高达4.75倍的速度提升。  
