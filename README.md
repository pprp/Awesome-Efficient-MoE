...
- MoTAS: MoE-Guided Feature Selection from TTS-Augmented Speech for   Enhanced Multimodal Alzheimer's Early Screening
  ...
  - Code: Not available
  - Summary: The research paper presents MoTAS, a novel framework that enhances early screening for Alzheimer's Disease (AD) by utilizing Text-to-Speech (TTS) augmentation and a Mixture of Experts (MoE) mechanism for effective multimodal feature selection. By synthesizing additional speech data and dynamically selecting informative features through MoE, the framework improves model generalization and achieves a leading accuracy of 85.71% on the ADReSSo dataset, surpassing existing methods. The study's findings underscore MoTAS's potential in addressing data limitations in real-world AD screening applications. #MixtureOfExperts #MoE #Efficiency #Alzheimers #SpeechRecognition
  - 摘要: 本研究提出了MoTAS框架，通过文本到语音（TTS）增强技术和混合专家（MoE）机制，旨在提高阿尔茨海默病（AD）早期筛查的效率。MoTAS利用TTS合成语音扩展数据集，并通过动态选择最具信息量的特征，优化多模态特征融合，从而提升模型的分类准确率。在ADReSSo数据集上的评估结果显示，MoTAS达到了85.71%的领先准确率，超越了现有基线，验证了TTS增强和MoE机制在分类性能提升中的重要贡献。#混合专家 #MoE #效率  - Authors: Yuan Xie, Shaohan Huang, Tianyu Chen, Furu Wei
