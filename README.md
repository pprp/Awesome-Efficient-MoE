...
- MoTAS: MoE-Guided Feature Selection from TTS-Augmented Speech for   Enhanced Multimodal Alzheimer's Early Screening
  ...
  - Code: Not available
  - Summary: The research paper presents MoTAS, a novel framework that enhances early screening for Alzheimer's Disease (AD) by integrating Text-to-Speech (TTS) augmentation and a Mixture of Experts (MoE) mechanism for effective multimodal feature selection. By using automatic speech recognition (ASR) for accurate transcriptions and synthesizing additional speech data, MoTAS significantly increases the dataset's volume and improves feature selection, leading to a classification accuracy of 85.71% on the ADReSSo dataset—surpassing existing methods. The study underscores the framework's potential in real-world applications, particularly in scenarios with limited data availability. #MixtureOfExperts #MoE #Efficiency #Alzheimers #MachineLearning
  - 摘要: 本文提出了一种名为MoTAS的框架，旨在通过语音进行阿尔茨海默病（AD）的早期筛查。该方法利用文本到语音（TTS）增强数据量，并结合混合专家（MoE）机制进行多模态特征选择，从而提高模型的泛化能力。经过在ADReSSo数据集上的评估，MoTAS达到了85.71%的准确率，超越了现有基线，验证了TTS增强和MoE在提升分类性能方面的独特贡献。 #混合专家 #MoE #效率  - Authors: Yuan Xie, Shaohan Huang, Tianyu Chen, Furu Wei
