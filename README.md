...
- Steering MoE LLMs via Expert (De)Activation
  ...
  - Code: Not available
  - Summary: The research paper introduces SteerMoE, a novel framework for steering Mixture-of-Experts (MoE) in Large Language Models (LLMs) by detecting and controlling behavior-linked experts through their distinct activation patterns. By selectively (de)activating these experts during inference, the framework enhances model behaviors such as safety and faithfulness by up to 20% and 27%, respectively, without the need for retraining. However, the study also reveals vulnerabilities, as the method can significantly reduce safety in adversarial settings, highlighting a new aspect of alignment challenges in MoE models. #MixtureOfExperts #MoE #LLM #AIAlignment #MachineLearning
  - 摘要: 本文提出了一种名为SteerMoE的框架，用于通过检测和控制与行为相关的专家来引导混合专家模型（MoE）。该方法通过识别在对比行为的输入对中具有不同激活模式的专家，实现了在推理过程中选择性地（去）激活这些专家，从而在不重新训练或修改权重的情况下控制模型的行为，如忠实性和安全性。实验结果表明，该方法在11个基准测试和6个大型语言模型上，安全性提高了最高20%，忠实性提高了27%。然而，在对抗攻击模式下，安全性单独下降了41%，与现有破解方法结合时则完全失效，揭示了隐藏在专家中的对齐伪装的新维度。
