...
- Wav2DF-TSL: Two-stage Learning with Efficient Pre-training and   Hierarchical Experts Fusion for Robust Audio Deepfake Detection
  ...
  - Code: Not available
  - Summary: The research paper presents Wav2DF-TSL, a two-stage learning framework designed for robust audio deepfake detection (ADD) by addressing the limitations of existing self-supervised learning models that primarily rely on real speech. The proposed method incorporates a pre-training stage utilizing adapters to learn from 3000 hours of unlabelled spoofed audio, enhancing feature adaptability while preventing catastrophic forgetting. In the fine-tuning stage, the hierarchical adaptive mixture of experts (HA-MoE) method is employed to effectively integrate multi-level spoofing cues, resulting in significant performance improvements, particularly on cross-domain datasets, with a 27.5% relative enhancement in equal error rate (EER) compared to state-of-the-art systems. #MixtureOfExperts #MoE #Efficiency
  - 摘要: 本文提出了一种基于预训练和层次专家融合的两阶段学习策略Wav2DF-TSL，以提高音频深伪检测的鲁棒性。通过在预训练阶段利用适配器高效学习3000小时无标签伪造语音的特征，增强前端特征的适应性，并在微调阶段采用层次自适应混合专家（HA-MoE）方法，通过多专家协作动态融合多层伪造线索。实验结果表明，该方法在四个基准数据集上显著优于基线系统，特别是在跨域In-the-wild数据集上实现了27.5%的相对改进，超越了现有最先进的系统。#混合专家 #MoE #效率#混合专家 #MoE #效率
