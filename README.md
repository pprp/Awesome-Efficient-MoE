...
- Super-Linear: A Lightweight Pretrained Mixture of Linear Experts for   Time Series Forecasting
  ...
  - Code: Not available
  - Summary: The research paper introduces Super-Linear, a lightweight and scalable mixture-of-experts (MoE) model designed for time series forecasting (TSF), which addresses the high computational costs associated with large pre-trained models like Chronos and Time-MoE. By utilizing frequency-specialized linear experts and a dynamic spectral gating mechanism, Super-Linear efficiently selects relevant experts for accurate forecasting across diverse datasets and sampling rates. The model achieves state-of-the-art performance while enhancing efficiency, robustness, and interpretability, making it a significant contribution to the field of TSF. #MixtureOfExperts #MoE #Efficiency
  - 摘要: 本文提出了一种轻量级的混合专家模型Super-Linear，旨在解决时间序列预测中的高计算成本问题。该模型通过使用简单的频率专用线性专家替代深度架构，并在多个频率范围内对重采样数据进行训练，从而实现高效而准确的预测。Super-Linear在保持与最先进模型相当的性能的同时，展现出更高的效率、对不同采样率的鲁棒性以及更好的可解释性。#混合专家 #MoE #效率#混合专家 #MoE #效率#混合专家 #MoE #效率
